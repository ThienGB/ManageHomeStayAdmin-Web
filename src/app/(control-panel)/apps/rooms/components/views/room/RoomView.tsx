												...
												render={({ field }) => (
															<Grid container spacing={2}>
																{amenities?.map((amenity: any) => {
																	...
																	const currentIds = (Array.isArray(current) ? current : []).map((v: any) => (typeof v === 'string' ? v : v?.id || v?._id || v));
																					...
																					<Checkbox
																						...
																						onChange={(e) => {
																							...
																							if (e.target.checked) {
																								...
																								if (!ids.includes(amenity.id)) ids.push(amenity.id);
																								field.onChange(ids);
																							} else {
																								// remove id
																								field.onChange(ids.filter((id: string) => id !== amenity.id));
																							}
																						}}
																					/>
																				}
